...
#define RETCODE_VALID( retcode ) ((retcode) == SE_RET_CODE_SUCCESS )
...
typedef struct
{
   ...
   /*
    * $$$ template_association is always true for valid <Variable> instance
    */
...
typedef struct
{
    SE_String                model_object_id_string;
    int                      model_index;
...
typedef struct
{
    union
    {
        ...
        SE_Spatial_Resolution_LOD_Data_Fields spatial_resolution_data;
 ...
 *
 *   name -- SE_String; the name of the entry
 *
 ...
 *
 *   Global store for GetFields and object ID processing
 */
extern SE_Store  store;

extern SE_Boolean file_output_option;

/* Functions */

extern void init_colour_table(void);
 ...
 *
 *    Given an SEDRIS texture ID, return a pointer to the texture's entry
 *
 *    If the texture is not found but textures were provided, return the
 *    first texture in global_texture_array.
 *
 *    If no textures were provided, return NULL.
 *
 * PARAMETERS:
 *
 ...
 * RETURNS:
 *
 *   NULL if no textures were provided
 *
 *   answer_ptr, a local pointer variable
 *
 *----------------------------------------------------------------------------
 */
extern TEXTURE_ARRAY_ENTRY *find_texture
(
...
extern void retrieve_textures(SE_Object transmittal_root, SE_Encoding imp_encoding );
extern void process_model_textures(void);
...
extern const char *get_texture_name(const SE_String *texture_object_id);
...
extern void set_callbacks(void);
extern void initialize_viewer(void);
extern void draw_frame(void);
extern void shutdown_viewer(void);

extern void handle_key_pressed(unsigned char, int, int);
extern void handle_special_key_pressed(int, int, int);
extern void mouse_click(int, int, int, int);
extern void mouse_motion(int, int);
extern void reshape(int, int);

extern void start_vert_tracking(void);
...
extern void calculate_root_model_extents
            (const SE_URL *transmittal_location, char *extents_file);
