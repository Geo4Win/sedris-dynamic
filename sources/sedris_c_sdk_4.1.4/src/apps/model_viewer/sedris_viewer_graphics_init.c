 ...
 */
void
initialize_graphics(int *argcp, char **argv)
{
    char *title = "SEDRIS OpenGL Model Viewer";
...
*/
    ...
    mainGlutWindowId = glutCreateWindow(title);
 ...
 *
 *   Sets up the callbacks that will drive the rest of the program.
 *
 * PARAMETERS:
 *
 *    none
 ...
 *----------------------------------------------------------------------------
 */
static void clear_graphics_states(void)
{
...
)
{
    ...
    switch (node_ptr->tag)
    {
        ...
        case MODEL_INSTANCE_NODE:
             ...
             {
                 fprintf(stderr, "Error - model_index = -1\n");
                 exit(-1);
             }
             create_lists_for_node
...
(
    const SE_String *texture_object_id
)
{
    ...
    {
        if (SE_CompareStrings(&(global_texture_array[i].texture_object_id),
            texture_object_id) == 0)
        {
    ...
    if (!found)
    {
        fprintf(stderr,"Error - unknown texture id %s\n",
                SE_GetStringAsCharString(texture_object_id));
        fprintf(stderr,"replacing with texture %s\n",
                SE_GetStringAsCharString
                (&(global_texture_array[0].texture_object_id)));
...
)
{
    ...
    * setting)
    */
    if (mfunc_ptr && (mfunc_ptr->texture_id_string.length > 0 ))
    {
 ...
 *                           SE_GetRearrangedImageData() to retrieve the
 *                           actual texels
 *
 ...
 *
 *    alpha_image_parms - parameters for the call to SE_GetRearrangedImageData()
 *
 *    rgb_image_parms   - parameters for the call to SE_GetRearrangedImageData()
 *
...
(
    SE_Object                   alpha_texture_object,
    SE_Object                   rgb_texture_object,
    GLint                       level,
...
{
    GLubyte             *ubyte_OpenGL_rgba_array;
    GLubyte             *ubyte_OpenGL_alpha_array;
    SE_Image_Data               imgData_alpha_array,
                                imgData_rgba_array;
    SE_Image_Texel_Location_3D  start_texel, stop_texel;
    SE_Status_Code  status;

    if (!(ubyte_OpenGL_rgba_array = (GLubyte *)
          calloc(rgba_byte_count, sizeof(GLubyte))))
    {
        fprintf(stderr, "Can't calloc memory for openGL " \
                "GLubyte texture array\n");
        exit(-1);
    }
    imgData_rgba_array.data = ubyte_OpenGL_rgba_array;
    imgData_rgba_array.data_count = rgba_byte_count;

    start_texel.horizontal = 0;
    start_texel.vertical   = 0;
    start_texel.z          = 0;
    stop_texel.horizontal  = width-1;
    stop_texel.vertical    = height-1;
    stop_texel.z           = 0;
    status = SE_GetRearrangedImageData
             (
                 rgb_texture_object,
                 &start_texel,
                 &stop_texel,
    ...
    {
        fprintf(stderr, "Error (%s) in SE_GetRearrangedImageData()" \
                " for merging RGB texture level %i\n",
                     SE_PrintStatusCode( status ), (int) level);
        exit(-1);
    }

    if (!(ubyte_OpenGL_alpha_array = (GLubyte *)
          calloc(texel_count, sizeof(GLubyte))))
    {
        fprintf(stderr, "Can't calloc memory for openGL " \
                "GLubyte texture array\n");
        exit(-1);
    }
    imgData_alpha_array.data = ubyte_OpenGL_alpha_array;
    imgData_alpha_array.data_count = texel_count;

    status = SE_GetRearrangedImageData
             (
                 alpha_texture_object,
                &start_texel,
                &stop_texel,
                 (SE_Short_Integer_Unsigned) level, /* mip level */
                 &alpha_image_parms,
    ...
    {
        fprintf(stderr, "Error (%s) in SE_GetRearrangedImageData()" \
                " for merging Alpha texture level %i\n",
                     SE_PrintStatusCode( status ), (int) level);
        exit(-1);
    }

    {
        ...
        *
        * (it would be nice if SE_GetRearrangedImageData() did this...)
        */
...
{
    SE_Object           rgb_texture_obj, rgb_mapping_func_obj;
    SE_Object           alpha_texture_obj, alpha_mapping_func_obj;
    SE_FIELDS_PTR       rgb_texture_fields_ptr;
    GLint               level;
    GLint               last_level_to_process;
    SE_Return_Code      result;
    ...
    result = SE_GetNthComponent
             (
                 pgon_object,
                 SE_CLS_DRM_IMAGE_MAPPING_FUNCTION,
                 rgb_image_function_index,
                 &rgb_mapping_func_obj,
                 NULL
             );

    if (result != SE_RETCOD_SUCCESS)
    {
        fprintf(stderr,"Can't get the RGB Mapping Function (%i) of a pgon\n",
                (int) rgb_image_function_index);
        fprintf(stderr,"Result = %i\n", (int) result);
        exit(-1);
    }

    result = SE_GetAssociate(rgb_mapping_func_obj,
                             SE_CLS_DRM_IMAGE,
                             SE_ITRBEH_RESOLVE,
                             &rgb_texture_obj,
                             NULL);
    ...
    result = SE_GetNthComponent
             (
                 ...
                 &alpha_mapping_func_obj,
                 NULL
             );

    if (result != SE_RETCOD_SUCCESS)
    {
        fprintf(stderr,"Can't get the Alpha Mapping Function (%i) of a pgon\n"\
                       "Result = %s\n", (int) alpha_image_function_index,
                SE_PrintReturnCode(result));
        exit(-1);
    }

    result = SE_GetAssociate(alpha_mapping_func_obj,
                             SE_CLS_DRM_IMAGE,
                             SE_ITRBEH_RESOLVE,
                             &alpha_texture_obj,
                             NULL);
    ...
    if (SE_GetFields(rgb_texture_obj, store, &rgb_texture_fields_ptr)
        != SE_RETCOD_SUCCESS)
    {
        fprintf(stderr, "Error - can't get the RGB Texture's fields!\n");
        exit(-1);
    }
    ...
    {
        fprintf(stderr,"Fatal Error - A texture with 0 levels!\n");
        exit(-1);
    }
    ...
    {
        fprintf(stderr,"Can't calloc memory for texture levels array\n");
        exit(-1);
    }
    ...
    * defintion for this merged image.  If this image has multiple levels
    ...
    */
    ...
    for (level = 0; level < last_level_to_process; level++)
    {
        ...
        printf("\nmerged texture - \n" \
               ...
               (int) level,
               (int) curr_level_width,
               (int) curr_level_height);
    }
    ...
    */
    ...
    else
    {
        ...
        */
        for (level = 0; level < merged_entry_ptr->levels; level++)
        {
            ...
            printf("\nmerged texture - \n" \
               "        - level %i (as its own image) => width %i, height %i\n",
               (int) level,
               (int) curr_level_width,
               (int) curr_level_height);
    ...
    if (SE_FreeObject(rgb_mapping_func_obj) != SE_RETCOD_SUCCESS)
        fprintf(stderr, "Error - Free RGB Mapping Function object failed!\n");

    if (SE_FreeObject(rgb_texture_obj) != SE_RETCOD_SUCCESS)
        fprintf(stderr, "Error - Free RGB Image object failed!\n");

    if (SE_FreeObject(alpha_mapping_func_obj) != SE_RETCOD_SUCCESS)
        fprintf(stderr, "Error - Free Alpha Mapping Function object failed!\n");

    if (SE_FreeObject(alpha_texture_obj) != SE_RETCOD_SUCCESS)
        fprintf(stderr, "Error - Free Alpha Image object failed!\n");
}
 ...
 *
 *   image_parms     - parameters for the call to SE_GetRearrangedImageData()
 *
...
{
    SE_Integer_Unsigned         byte_count, texel_count;
    SE_Status_Code              status;
    GLubyte                    *ubyte_OpenGL_array = NULL;
    SE_Image_Data               imgData_array;
    SE_Image_Texel_Location_3D  start_texel, stop_texel;

    texel_count = texture_entry->level_array[level].width *
                  texture_entry->level_array[level].height;

    byte_count = texel_count * component_count;

    if (!(ubyte_OpenGL_array = (GLubyte *)
          calloc(byte_count, sizeof(GLubyte))))
    {
        fprintf(stderr, "Can't calloc memory for openGL " \
                "GLubyte texture array\n");
        exit(-1);
    }
    imgData_array.data = ubyte_OpenGL_array;
    imgData_array.data_count = byte_count;
    start_texel.horizontal = 0;
    start_texel.vertical   = 0;
    start_texel.z          = 0;
    stop_texel.horizontal  = texture_entry->level_array[level].width-1;
    stop_texel.vertical    = texture_entry->level_array[level].height-1;
    stop_texel.z           = 0;
    status = SE_GetRearrangedImageData
             (
                 texture_obj,
    ...
    {
        fprintf(stderr, "Error (%s) in SE_GetRearrangedImageData()" \
                " for texture %s, level %i\n",
                SE_PrintStatusCode(status),
                (char *)texture_entry->name.characters, (int) level);
    }
        ...
        */
        ...
        for (i=0; i<texture_entry->level_array[level].height; i++)
        {
            for (j=0; j<texture_entry->level_array[level].width; j++)
            {
                 ...
                 * set to 0 (black) by SE_GetRearrangedImageData(), and for
...
)
{
    switch (image_signature)
    {
        ...
        default:
             fprintf(stderr,"Unknown image signature %i\n",
                 (int) image_signature);
             break;
    }
}
...
)
{
    ...
    if (SE_GetFields(texture_obj, store, &texture_fields_ptr) != SE_RETCOD_SUCCESS)
    {
        fprintf(stderr, "Error - can't get the Image's fields!\n");
        exit(-1);
    }

    if (SE_CloneString(&(texture_fields_ptr->u.Image.name),
        &(texture_entry->name)) != SE_DRM_STAT_CODE_SUCCESS)
    {
        fprintf(stderr, "create_openGL_texture_list: can't clone name\n");
        fflush(stderr);
    }
    if (SE_CloneString(obj_ID_string,
        &(texture_entry->texture_object_id)) != SE_DRM_STAT_CODE_SUCCESS)
    {
        fprintf(stderr, "create_openGL_texture_list: can't clone object id\n");
        fflush(stderr);
    }
   ...
   /*
    * Create the OpenGL display list which is the 'master' texture
    * defintion for this image.  If this image has multiple levels and
    ...
    */
    ...
    for (level = 0; level < last_level_to_process; level++)
    {
        ...
        printf("\ntexture %s - %s\n" \
               ...
               "        - %stransparent\n" \
               "        - image signature = %s\n",
               SE_GetStringAsCharString(&(texture_entry->texture_object_id)),
               SE_GetStringAsCharString(&(texture_entry->name)),
               (int) level,
               (int) texture_entry->level_array[level].width,
               (int) texture_entry->level_array[level].height,
               texture_entry->transparent ? "is " : "is not ",
...
retrieve_textures
(
    SE_Object transmittal_root,
    SE_Encoding imp_encoding
)
{
    SE_Integer_Unsigned  i;
    SE_Object            image_library_obj = NULL,
                         image_obj = NULL;
    SE_Store             tmp_store = NULL;
    SE_String            object_ID_string;

    if (SE_CreateStore(imp_encoding, &tmp_store) != SE_RETCOD_SUCCESS)
    {
        fprintf(stderr, "Error - can't create store!\n");
        exit(-1);
    }
    global_texture_count = 0;
    if (SE_GetComponent(transmittal_root, SE_CLS_DRM_IMAGE_LIBRARY,
        SE_TRUE, SE_TRUE, SE_ITRBEH_RESOLVE, &image_library_obj, NULL)
        == SE_RETCOD_SUCCESS)
    {
        while (SE_GetNthComponent(image_library_obj, SE_CLS_DRM_IMAGE,
               global_texture_count+1, &image_obj, NULL) == SE_RETCOD_SUCCESS)
        {
            if (SE_FreeObject(image_obj) != SE_RETCOD_SUCCESS)
                fprintf(stderr, "Error - Free Image object failed!\n");
    ...
    {
        fprintf(stderr, "Warning - can't find an <Image Library>, "\
                "turning off texture processing\n");
        global_process_textures = SE_FALSE;
    }

    printf("\nThere are %i SEDRIS Images in the Transmittal.\n",
            (int) global_texture_count);

    if (global_texture_count > 0)
    {
       /*
        ...
        */
        if (global_texture_count)
        {
            ...
            {
                fprintf(stderr,"Can't calloc memory for texture array\n");
                exit(-1);
            }
        }
        ...
        {
            if( SE_GetNthComponent
                       (image_library_obj, SE_CLS_DRM_IMAGE,
                        i+1, &image_obj, NULL) == SE_RETCOD_SUCCESS )
            {
                if (SE_GetObjectIDString(image_obj, tmp_store,
                    &object_ID_string) != SE_RETCOD_SUCCESS)
                {
                      fprintf(stderr, "Error - can't get Object ID string!\n");
                      exit(-1);
                }
                create_openGL_texture_list
                (
                    &global_texture_array[i],
                    image_obj,
                   &object_ID_string
                );
            }
            ...
            {
                fprintf(stderr,
                        "Error - can't get the next Image (%i)!\n", (int) i);
                exit(-1);
            }
    ...
    if (image_library_obj)
        SE_FreeObject(image_library_obj);
} /* end retrieve_textures */
 ...
 *----------------------------------------------------------------------------
 */
void process_model_textures(void)
{
    SE_Integer_Unsigned i;
...
)
{
    ...
    for (i = 0; !answer_ptr && i < global_texture_count; i++)
    {
        if (SE_CompareStrings(&(global_texture_array[i].texture_object_id),
            texture_object_id) == 0)
        {
            answer_ptr = (char *)global_texture_array[i].name.characters;
        }
    }
    return answer_ptr;
}
...
)
{
    ...
    if (!global_ambient_colour_table)
    {
        ...
        {
            fprintf(stderr,"Can't calloc memory for ambient colour table\n");
            exit(-1);
        }
    }
    ...
    if (ix >= global_ambient_colour_count)
    {
                 ...
                 ix == global_ambient_colour_count)
        {
            ...
            */
            ...
            {
                fprintf(stderr,"Can't calloc memory for new ambient " \
                        "colour table\n");
                exit(-1);
            }
        ...
        else
        {
            ...
            */
            ix = 0;
            fprintf(stderr,"ERROR: Colour error - can't add colour " \
                    "(%f, %f, %f, %f)!\n", rgb.red, rgb.green,
                     rgb.blue, alpha);
        }
    }
...
)
{
    ...
    while (!found && curr_ptr)
    {
        if ((SE_CompareStrings
             (&(curr_ptr->alpha_texture_entry_ptr->texture_object_id),
              &(alpha_texture_ptr->texture_object_id)) == 0)
            &&
            (SE_CompareStrings
             (&(curr_ptr->rgb_texture_entry_ptr->texture_object_id),
    ...
    if (!found)
    {
        ...
        {
            fprintf(stderr,"Can't calloc memory for Merged Texture\n");
            exit(-1);
        }
