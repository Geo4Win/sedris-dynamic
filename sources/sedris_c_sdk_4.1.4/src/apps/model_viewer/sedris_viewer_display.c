...
static SE_Boolean         global_display_textures = SE_TRUE;
                         /*
...
)
{
    ...
    {
        fprintf(stderr,"Illegal drawing mode value of %i\n",
    ...
    if (current_drawing_mode != new_drawing_mode)
    {
        ...
        switch (new_drawing_mode)
        {
            ...
            default:
                 fprintf(stderr,"Impossible drawing mode value of %i\n",
                         (int) new_drawing_mode);
                 exit(-1);
        }
    ...
    */
    ...
    else if (current_mode != new_mode)
    {
        switch(new_mode)
        {
            ...
            default:
                 fprintf(stderr,"Illegal texture mode value of %i\n",
                         (int) new_mode);
                 new_mode = SE_IMAGMAPMETH_MODULATE;
                 fprintf(stderr,"Assuming MODULATE texture mode (%i).\n",
                         (int) new_mode);
                 set_drawing_mode(NOT_DRAWING);
                 glTexEnvi(GL_TEXTURE_ENV,GL_TEXTURE_ENV_MODE, GL_MODULATE);
                 break;
...
)
{
    ...
    else if (current_wrap_s_style != new_wrap_s_style)
    {
        ...
        switch(current_wrap_s_style)
        {
            ...
            default:
                 fprintf(stderr,"Illegal texture S wrapping value of %i\n",
                         (int) current_wrap_s_style);
                 current_wrap_s_style = SE_IMAGWRAP_REPEAT;
                 fprintf(stderr,"Assuming REPEAT texture S wrapping (%i).\n",
                         (int) current_wrap_s_style);
                 set_drawing_mode(NOT_DRAWING);
                 glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
                 break;
   ...
   else if (current_wrap_t_style != new_wrap_t_style)
   {
       ...
       */
       ...
       switch(current_wrap_t_style)
       {
           ...
           default:
                fprintf(stderr,"Illegal texture T wrapping value of %i\n",
                        (int) current_wrap_t_style);
                current_wrap_t_style = SE_IMAGWRAP_REPEAT;
                fprintf(stderr,"Assuming REPEAT texture T wrapping (%i).\n",
                        (int) current_wrap_t_style);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
                break;
       }
...
)
{
    ...
    switch(update_colour_buffers)
    {
        ...
        default: /* treat illegal value as an UPDATE_COLOUR_BUFFER */
             fprintf(stderr,"Illegal parameter (%i) to set_colour_masks()\n",
...
)
{
    ...
    switch(update_depth_buffer)
    {
        ...
        default: /* treat illegal value as an UPDATE_DEPTH_BUFFER */
             fprintf(stderr,"Illegal parameter (%i) to set_depth_mask()\n",
...
)
{
    ...
    if (p_ptr->concave == SE_FALSE)
    {
        ...
        */
        ...
        {
            if (do_textures &&
                (texture_coords_index < p_ptr->vertex_array->texture_coord_count))
...
)
{
    ...
    switch (expr_ptr->expression_type)
    {
        case PREDEFINED_FUNCTION_EXPR:
             fprintf(stderr,"Warning - function evaluation not supported\n");
    ...
    switch (expr_ptr->expression_type)
    {
        ...
        case VARIABLE_EXPR:
             if (EDCS_AttributeValueToLongFloat(&(expr_ptr->u.variable.value),
                 &answer) != EDCS_SC_SUCCESS)
             {
                 fprintf(stderr,
                         "Error - unknown variable.attribute_value_type (%i)\n",
            ...
            switch (xform_ptr->xform_array[i].tag)
            {
                ...
                case ROTATION_XFORM:
                     if (xform_ptr->xform_array[i].control_link_ptr)
                     {
                         ...
                         {
                             fprintf(stderr,"Internal Model Viewer error - " \
...
)
{
    ...
    */
    ...
    if (!found)
    {
        fprintf(stderr, "Model Viewer internal error - Model %s has " \
                " a %s state which we missed.\n",
                SE_GetStringAsCharString
...
)
{
    ...
    {
        case NULL_NODE:
             fprintf(stderr,"Warning - current model (%s) has a NULL node.\n",
                     SE_GetStringAsCharString
                     (&(g_models[viewer_parms.model_index].name)));
             break;
        ...
        case ANIMATION_RELATED_NODE:
             fprintf(stderr,
                     "Warning - current model (%s) has an Animation node; "\
                     "not drawn.\n",
                     SE_GetStringAsCharString
                     (&(g_models[viewer_parms.model_index].name)));
             break;
     ...
     switch (val_ptr->attribute_value_type)
     {
         ...
         case EDCS_AVT_INTEGER:
              switch( val_ptr->value.integer_value.numeric_value_type )
              {
                ...
                default:
                    /*
                     * this should actually never happen, since the numeric value type
                     * is set by model_viewer
                     */
                     fprintf(stderr,
                       "Still need to fix the interval integer values in %s line %d\n",
                       __FILE__, __LINE__);
                    break;
              }
              break;
         ...
         case EDCS_AVT_COUNT:
              switch( val_ptr->value.count_value.numeric_value_type )
              {
                ...
                default:
                    /*
                     * this should actually never happen, since the
                     * numeric value type is set by model_viewer
                     * fprintf(stderr,
                     *   "Still need to fix the interval integer values in "\
                     *   "%s line %d\n", __FILE__, __LINE__);
                     */
                    break;
              }
         ...
         case EDCS_AVT_REAL:
              switch (val_ptr->value.real_value.numeric_value_type)
              {
                  ...
                  default:
                        ...
                        * this should actually never happen, since the
                        * numeric value type is set by model_viewer
                        * fprintf(stderr, "Still need to fix the "\
                        *         "interval float values in %s line %d\n",
                        *       __FILE__, __LINE__);
                        */
                        break;
              }
         ...
         case EDCS_AVT_STRING:
              fprintf(stderr,"Warning - trying to add an int to "\
 ...
 */
void
calculate_root_model_extents
(
    const SE_URL *transmittal_location,
          char   *extents_file
)
{
    SE_Integer_Unsigned  model_index;
    NODE_EXTENTS        *extents = NULL;
    GLfloat              hgt, len, wid;
    GLfloat              x_dim, y_dim;
    FILE                *ext_out = NULL;
    ...
    if (extents_file)
    {
        ...
        {
            fprintf(stderr,
                "Could not open %s for writing out root model extents\n",
                extents_file);
        }
        else
        {
            fprintf(stderr,
                "Opened %s for writing out root model extents\n",
                extents_file);
        }
    }
    ...
    fprintf(stderr,"#\n");
    fprintf(stderr,"#%-20s\theight\tlength\twidth\n", "model_name");
    fprintf(stderr,"#\n");

    if (ext_out)
    {
        fprintf(ext_out, "######## Root model extents for STF: %s ########\n",
                (char *)transmittal_location->characters);
        fprintf(ext_out,"#\n");
        fprintf(ext_out,"#%-20s\theight\tlength\twidth\n", "model_name");
        fprintf(ext_out,"#\n");

    }

    for (model_index = 0; model_index < global_model_count; model_index++)
    {
...
} /* end calculate_root_model_extents */
...
)
{
    if (mfunc_ptr)
    {
        ...
        if (mfunc_ptr->texture_id_string.length > 0)
        {
            ...
            printf("polygon %i %ss %s texture (id=%s) (signature=%s)\n",
                   ...
                   SE_GetStringAsCharString(&(mfunc_ptr->texture_id_string)),
...
)
{
    ...
    switch (node_ptr->tag)
    {
        ...
        case MODEL_INSTANCE_NODE:
             ...
             print_indent(indent);
             printf("  id = %s\n",
                    SE_GetStringAsCharString(&(node_ptr->u.model_inst_data.
        ...
        case STATE_RELATED_NODE:
             ...
             printf("  default state value = %s\n",
                       buffer_state_value(&node_ptr->u.state_related_data.default_state_value,
 ...
 *----------------------------------------------------------------------------
 */
static double get_closest_lod_range(void)
{
    double answer = 0.0;
...
{
    switch (ea_value.attribute_value_type)
    {
        case EDCS_AVT_BOOLEAN:
             printf("%s", ea_value.value.boolean_value == EDCS_TRUE ? "True" : "False");
             break;

        case EDCS_AVT_INTEGER:
        {
            switch( ea_value.value.integer_value.numeric_value_type )
            {
                case EDCS_NVT_SINGLE_VALUE:
                     printf("%i", (long) ea_value.value.integer_value.value.single_value);
                     break;
                case EDCS_NVT_OPEN_INTERVAL:
                     printf("%i to %i",
                     (long) ea_value.value.integer_value.value.open_interval.lower_bound,
                     (long) ea_value.value.integer_value.value.open_interval.upper_bound );
                     break;
                case EDCS_NVT_GE_LT_INTERVAL:
                     printf("%i to %i",
                     (long) ea_value.value.integer_value.value.ge_lt_interval.lower_bound,
                     (long) ea_value.value.integer_value.value.ge_lt_interval.upper_bound);
                     break;
                case EDCS_NVT_GT_LE_INTERVAL:
                     printf("%i to %i",
                     (long) ea_value.value.integer_value.value.gt_le_interval.lower_bound,
                     (long) ea_value.value.integer_value.value.gt_le_interval.upper_bound);
                     break;
                case EDCS_NVT_CLOSED_INTERVAL:
                     printf("%i to %i",
                     (long) ea_value.value.integer_value.value.closed_interval.lower_bound,
                     (long) ea_value.value.integer_value.value.closed_interval.upper_bound);
                     break;
                case EDCS_NVT_GT_SEMI_INTERVAL:
                     printf("%i to infinity",
                            (long) ea_value.value.integer_value.value.gt_semi_interval);
                     break;
                case EDCS_NVT_GE_SEMI_INTERVAL:
                     printf("%i to infinity",
                            (long) ea_value.value.integer_value.value.ge_semi_interval);
                     break;
                case EDCS_NVT_LT_SEMI_INTERVAL:
                     printf("-infinity to %i",
                            (long) ea_value.value.integer_value.value.lt_semi_interval);
                     break;
                case EDCS_NVT_LE_SEMI_INTERVAL:
                     printf("-infinity to %i",
                            (long) ea_value.value.integer_value.value.le_semi_interval);
                     break;
            }
            break;
        }
        case EDCS_AVT_COUNT:
        {
            switch( ea_value.value.count_value.numeric_value_type )
            {
                case EDCS_NVT_SINGLE_VALUE:
                     printf("%i", (long) ea_value.value.count_value.value.single_value);
                     break;
                case EDCS_NVT_OPEN_INTERVAL:
                     printf("%i to %i",
                     (long) ea_value.value.count_value.value.open_interval.lower_bound,
                     (long) ea_value.value.count_value.value.open_interval.upper_bound );
                     break;
                case EDCS_NVT_GE_LT_INTERVAL:
                     printf("%i to %i",
                     (long) ea_value.value.count_value.value.ge_lt_interval.lower_bound,
                     (long) ea_value.value.count_value.value.ge_lt_interval.upper_bound );
                     break;
                case EDCS_NVT_GT_LE_INTERVAL:
                     printf("%i to %i",
                     (long) ea_value.value.count_value.value.gt_le_interval.lower_bound,
                     (long) ea_value.value.count_value.value.gt_le_interval.upper_bound );
                     break;
                case EDCS_NVT_CLOSED_INTERVAL:
                     printf("%i to %i",
                     (long) ea_value.value.count_value.value.closed_interval.lower_bound,
                     (long) ea_value.value.count_value.value.closed_interval.upper_bound );
                     break;
                case EDCS_NVT_GT_SEMI_INTERVAL:
                     printf("%i to infinity",
                            (long) ea_value.value.count_value.value.gt_semi_interval);
                     break;
                case EDCS_NVT_GE_SEMI_INTERVAL:
                     printf("%i to infinity",
                            (long) ea_value.value.count_value.value.ge_semi_interval);
                     break;
                case EDCS_NVT_LT_SEMI_INTERVAL:
                     printf("-infinity to %i",
                            (long) ea_value.value.count_value.value.lt_semi_interval);
                     break;
                case EDCS_NVT_LE_SEMI_INTERVAL:
                     printf("-infinity to %i",
                            (long) ea_value.value.count_value.value.le_semi_interval);
                     break;
            }
            break;
        }
        case EDCS_AVT_REAL:
        {
            switch( ea_value.value.real_value.numeric_value_type )
            {
                case EDCS_NVT_SINGLE_VALUE:
                     printf("%f", (double) ea_value.value.real_value.value.single_value);
                     break;
                case EDCS_NVT_OPEN_INTERVAL:
                     printf("%f to %f",
                     (double) ea_value.value.real_value.value.open_interval.lower_bound,
                     (double) ea_value.value.real_value.value.open_interval.upper_bound );
                     break;
                case EDCS_NVT_GE_LT_INTERVAL:
                     printf("%f to %f",
                     (double) ea_value.value.real_value.value.ge_lt_interval.lower_bound,
                     (double) ea_value.value.real_value.value.ge_lt_interval.upper_bound );
                     break;
                case EDCS_NVT_GT_LE_INTERVAL:
                     printf("%f to %f",
                     (double) ea_value.value.real_value.value.gt_le_interval.lower_bound,
                     (double) ea_value.value.real_value.value.gt_le_interval.upper_bound );
                     break;
                case EDCS_NVT_CLOSED_INTERVAL:
                     printf("%f to %f",
                     (double) ea_value.value.real_value.value.closed_interval.lower_bound,
                     (double) ea_value.value.real_value.value.closed_interval.upper_bound );
                     break;
                case EDCS_NVT_GT_SEMI_INTERVAL:
                     printf("%f to infinity",
                         (double) ea_value.value.real_value.value.gt_semi_interval);
                     break;
                case EDCS_NVT_GE_SEMI_INTERVAL:
                     printf("%f to infinity",
                           (double) ea_value.value.real_value.value.ge_semi_interval);
                     break;
                case EDCS_NVT_LT_SEMI_INTERVAL:
                     printf("-infinity to %f",
                            (double) ea_value.value.real_value.value.lt_semi_interval);
                     break;
                case EDCS_NVT_LE_SEMI_INTERVAL:
                     printf("-infinity to %f",
                            (double) ea_value.value.real_value.value.le_semi_interval);
                     break;
            }
            break;
        }
        ...
        case EDCS_AVT_KEY:
        case EDCS_AVT_CONSTRAINED_STRING:
             printf("%s", ea_value.value.string_value.characters);
             break;

        case EDCS_AVT_ENUMERATION:
             printf("%s", EDCS_PrintEnumerantCode(ea_code,
                    ea_value.value.enumeration_value));
             break;
...
)
{
    ...
    {
        printf("ONLY LOD range = %.2f\n", model_entry_ptr->lod_ranges_array[0]);
    }
    else if (model_entry_ptr->discrete_lod_count==0)
    {
        printf("(model has NO LOD ranges)\n");
    }

    printf("\nTemplate Variable Count: %i\n",
    ...
    {
        printf("Template Variable %i:\n", i+1);
        printf("    Variable value.value_type = ");
    ...
    for (i=0; i<model_entry_ptr->expression_count; i++)
    {
        ...
        switch(model_entry_ptr->expression_array[i].expression_type)
        {
            case PREDEFINED_FUNCTION_EXPR:
            case PSEUDO_CODE_FUNCTION_EXPR:
                 printf("    expression type: Function\n");
                 break;

            case LITERAL_EXPR:
                 printf("    expression type: Literal\n");
                 printf("    Literal value.value_type = ");
            ...
            default:
                 fprintf(stderr,"Illegal expression type %i\n",
    ...
    for (i=0; i<model_entry_ptr->control_link_count; i++)
    {
        ...
        switch(model_entry_ptr->control_link_array[i].control_link_type)
        {
            ...
            default:
                 fprintf(stderr,"Illegal Control Link type %i\n",
    ...
    printf("rotation delta = %i degree%s/frame\n",
            global_control_variable_delta,
            (global_control_variable_delta != 1) ? "s" : "");

    printf("State Tag Count: %i\n",
           (int) model_entry_ptr->state_tag_count);

    printf("Current State Tag Index: %i\n",
           (int) model_entry_ptr->current_state_tag_index);

    if (model_entry_ptr->state_tag_count) printf("\n");
    ...
    if (global_display_textures)
        printf("Displaying Textures\n");
    else
        printf("Not Displaying Textures\n");
} /* end print_model */
 ...
 *----------------------------------------------------------------------------
 */
static void print_model_summary(void)
{
    printf("\nmodel (%i of %i) = %s (%i levels)\n",
       viewer_parms.model_index+1, global_model_count,
       SE_GetStringAsCharString(&(g_models[viewer_parms.model_index].name)),
 ...
 *----------------------------------------------------------------------------
 */
static void next_merged_texture(void)
{
 ...
 *----------------------------------------------------------------------------
 */
static void prev_merged_texture(void)
{
 ...
 */
static void
print_texture_summary(void)
{
    SE_Short_Integer mip_level = 0;

    mip_level = global_texture_array[viewer_parms.texture_index].current_level;

    printf("\nviewer_parms.texture_index (%i of %i) =\n" \
           "    %s\n" \
           "    %i by %i at level %u, %i levels,\n" \
           "    %stransparent\n" \
           "    image signature = %s\n",
 ...
 */
static void
print_merged_texture_summary(void)
{
    ...
    printf("\nmerged texture (%i of %i) =\n" \
           "    merged SEDRIS RGB texture ID %s (%s)\n" \
           "    merged SEDRIS A   texture ID %s (%s)\n" \
           "    %i by %i at level %u, %i levels,\n" \
           "    master openGL display list is %i\n" \
           "    current openGL display list is %i\n",
 ...
 *----------------------------------------------------------------------------
 */
static void set_global_range(void)
{
 ...
 *----------------------------------------------------------------------------
 */
static void set_current_state_value(void)
{
    SE_Integer_Unsigned   state_index;
    ...
    switch (default_state_value_ptr->attribute_value_type)
    {
        ...
        case EDCS_AVT_INTEGER:
        {
            switch( default_state_value_ptr->value.integer_value.numeric_value_type )
            {
                ...
                default:
                {
                    fprintf( stderr, "Anything other than single values not implemented in set_current_state_value()." );
                    break;
                }
            }
            break;
        ...
        default:
        {
            printf("invalid state type for set_current_state_value\n");
            break;
        }
    }
 ...
 *----------------------------------------------------------------------------
 */
static void print_lod_summary(void)
{
    SE_Integer_Unsigned lod_index;
    ...
    printf("Current Level of Detail index = %u\n", (unsigned) lod_index);

    printf("Current range  = %.2f  ", global_lod_range);
...
)
{
    ...
    switch (key)
    {
             ...
             */
             switch (global_polygon_style)
             {
                 case FILL:
                      ...
                      printf("Polygon style is WIREFRAME\n");
                      break;

                 case WIREFRAME:
                      global_polygon_style = OUTLINE;
                      printf("Polygon style is OUTLINE\n");
                      break;

                 case OUTLINE:
                      global_polygon_style = FILL;
                      printf("Polygon style is FILL\n");
                      break;
        ...
        case 'D':
             ...
             if (debug)
                 printf("debug = SE_TRUE\n");
             else
                 printf("debug = SE_FALSE\n");
             break;
        ...
        case 'd':
             ...
             */
             ...
             if (viewer_parms.use_lists)
                 printf("Using OpenGL Display Lists\n");
             else
                 printf("Not using OpenGL Display Lists\n");
             break;
        ...
        case 'E':
             ...
             */
             if(viewer_parms.draw_extents == SE_TRUE)
             {
               viewer_parms.draw_extents = SE_FALSE;
             }
             else
             {
               viewer_parms.draw_extents = SE_TRUE;
             }

             if (viewer_parms.draw_extents)
             {
                 printf("Drawing extents boxes:\n"
                        "   yellow = root model, purple = components\n");
             }
             else
             {
                 printf("Not drawing extents boxes\n");
             }
             break;
        ...
        case 'F':
             ...
             */
             ...
             printf("Fast texture (nearest filtering)\n");
             printf("     Texture Magnification Filter = %s\n",
                    filter_name(global_mag_filter));
             printf("     Texture Minification Filter  = %s\n",
                    filter_name(global_basic_min_filter));
             printf("     MIP Texture Minification Filter  = %s\n",
        ...
        case 'I':
             ...
             */
             if (viewer_parms.display_state == DISPLAY_MODELS)
             {
                 ...
                 {
                     printf("this model has no states\n");
                 }
                 else
                 {
                     SE_Integer_Unsigned     state_tag_index;
        ...
        case 'M':
             ...
             if (use_magic)
                 printf("use_magic = SE_TRUE\n");
             else
                 printf("use_magic = SE_FALSE\n");
             break;
        ...
        case 'm':
             ...
             printf("Increasing ambient light to %.2f %.2f %.2f\n",
        ...
        case 'N':
             ...
             global_inf_light_ambient[1] = global_inf_light_ambient[2] =
                 global_inf_light_ambient[0];
             printf("Decreasing ambient light to %.2f %.2f %.2f\n",
                     (double) global_inf_light_ambient[0],
                     (double) global_inf_light_ambient[1],
                     (double) global_inf_light_ambient[2]);
             glLightModelfv(GL_LIGHT_MODEL_AMBIENT, global_inf_light_ambient);
        ...
        case 'p':
             ...
             if (viewer_parms.display_state == DISPLAY_TEXTURES)
             {
                 printf("Display Mode = Texture Display Mode\n");
             }
             else if (viewer_parms.display_state == DISPLAY_MERGED_TEXTURES)
             {
                 printf("Display Mode = Merged Texture Display Mode\n");
             }
             else
             {
                 printf("Display Mode = Model Display Mode\n");
                 ...
                 if (g_models[viewer_parms.model_index].discrete_lod_count > 0)
                     printf("Current LOD Index = %u of %u\n",
             ...
             printf("\tPoint Light: %s\n", viewer_parms.point_light ?
                    "ON" : "OFF");
             printf("\tLight Model Viewing %s:\n", viewer_parms.local_viewing ?
                    "local" : "infinite");
             printf("\t2 Sided-Lighting: %s\n", viewer_parms.two_side_lighting
                    ? "ON" : "OFF");
             printf("\tShading: %s\n", viewer_parms.shading ? "Smooth":"Flat");
             if (viewer_parms.fast_textures)
                 printf("\tTexture speed: Fast (nearest filtering)\n");
             else
                 printf("\tTexture speed: Slow (linear filtering)\n");
        ...
        case 'S':
             ...
             */
             ...
             printf("Slow texture (linear filtering)\n"\
                    "     Texture Magnification Filter = %s\n",
                    filter_name(global_mag_filter));
             printf("     Texture Minification Filter  = %s\n",
                    filter_name(global_basic_min_filter));
             printf("     MIP Texture Minification Filter  = %s\n",
        ...
        case 'T':
             ...
             */
             ...
             if (global_double_draw_transparencies)
                 printf("\tDouble drawing to handle transparencies\n");
             else
                 printf("\tNOT Double drawing to handle transparencies\n");
             break;
        ...
        case '': /* ASCII code for escape key */
             shutdown_viewer();
             break;
 ...
 *----------------------------------------------------------------------------
 */
void handle_special_key_pressed
(
    int key,
    int x,
    int y
        ...
        case GLUT_KEY_F1:
             ...
             */
             switch (viewer_parms.display_state)
             {
                 ...
                 default:
                      fprintf(stderr,
                              "FATAL ERROR: Unknown display state = %i\n",
        ...
        case GLUT_KEY_F12:
             ...
             */
             if (global_process_textures)
             {
                 if (global_display_textures)
                 {
                     global_display_textures = SE_FALSE;
                     printf("Not Displaying Textures\n");
                 }
                 else
                 {
                     global_display_textures = SE_TRUE;
                     printf("Displaying Textures\n");
                 }
             }
             else
                 printf("Textures disabled at startup!\n");
 ...
 *
 *    Initialize texture processing (on or off).
 *
 *    Print out information on the first model being displayed.
 *
 * PARAMETERS:
 *
 *   none
