 ...
 *
 * FILE:        se_read1_image.c
 *
 * PROGRAMMERS: Bill Horan (SAIC), Howard Lu (SAIC), Warren Macchi (AcuSoft),
 *              Felix Rivera (AcuSoft)
 *
 * DESCRIPTION:
 *   Useful Utility Functions for <Images>
 *
...
)
{
    ...
    SE_Status_Code       status = SE_STATCODE_SUCCESS;
    SE_Store             flds_store = NULL, img_store = NULL;
    SE_FIELDS_PTR        fields_ptr = NULL;
    SE_Image_Data        image_data_src;
    SE_Integer_Unsigned  src_bytes;

    if (!data_out_ptr || !desired_img_params_ptr ||
        !start_texel || !stop_texel)
    {
        fprintf(stderr, "SE_GetRearrangedImageData: null input arg\n");
        status = SE_STATCODE_INACTIONABLE_FAILURE;
    }
    else if (SE_GetEncoding(image, &encoding) != SE_RETCOD_SUCCESS ||
        SE_CreateStore(encoding, &flds_store) != SE_RETCOD_SUCCESS ||
        SE_GetFields(image, flds_store, &fields_ptr) != SE_RETCOD_SUCCESS ||
        SE_IsA[fields_ptr->tag][SE_CLS_DRM_IMAGE] == SE_FALSE ||
        SE_ValidFields(fields_ptr, SE_FALSE, NULL) == SE_FALSE )
    {
        fprintf(stderr,
                "SE_GetRearrangedImageData: error getting fields for image\n");
        status = SE_STATCODE_INACTIONABLE_FAILURE;
    }
    else if ((SE_ValidImageExtents((&fields_ptr->u.Image),
               start_texel, stop_texel, mip_level) == SE_FALSE) ||
             (SE_ValidDesiredImageParameters(desired_img_params_ptr,
                 SE_FALSE, NULL ) == SE_FALSE))
    {
        fprintf(stderr, "SE_GetRearrangedImageData: invalid image extents\n");
        status = SE_STATCODE_INACTIONABLE_FAILURE;
    }
    else
    {
        SE_Image_Texel_Location_3D temp_start_texel, temp_stop_texel;

        temp_start_texel.horizontal = 0;
        temp_start_texel.vertical   = 0;
        temp_start_texel.z          = start_texel->z;

        temp_stop_texel.horizontal = fields_ptr->u.Image.
                                     mip_extents_array[mip_level].
                                     size_horizontal - 1;
        temp_stop_texel.vertical   = fields_ptr->u.Image.
                                     mip_extents_array[mip_level].
                                     size_vertical - 1;
        temp_stop_texel.z          = stop_texel->z;
       /*
        * Compute the size of the entire mip_level of the source image so
        * that we can allocate memory to hold it.
        *
        * Memory allocation for the source & temp image sections:
        *  - image_data_src will hold the original image section
        */
        if (SE_GetSizeOfImageData(image, &temp_start_texel,
              &temp_stop_texel, mip_level, &src_bytes) != SE_STATCODE_SUCCESS)
        {
            fprintf(stderr,
                    "SE_GetRearrangedImageData: can't get image data size\n");
            status = SE_STATCODE_INACTIONABLE_FAILURE;
        }
        else if ((SE_CreateStore(encoding, &img_store) != SE_RETCOD_SUCCESS)
              || (SE_GetImageData(image, &temp_start_texel, &temp_stop_texel,
                  mip_level, img_store, &image_data_src) != SE_RETCOD_SUCCESS))
        {
            fprintf(stderr, "SE_GetRearrangedImageData: "\
                    "error getting image data\n");
            status = SE_STATCODE_INACTIONABLE_FAILURE;
        }
        else
        {
            status = SE_RearrangeImageData (&fields_ptr->u.Image,
                      &image_data_src, start_texel, stop_texel,
                      mip_level, desired_img_params_ptr, data_out_ptr);
        }
    }
    if (img_store) SE_FreeStore(img_store);
    if (flds_store) SE_FreeStore(flds_store);

    return status;
} /* end SE_GetRearrangedImageData */
...
)
{
          ...
          SE_Store         store = NULL;
          SE_FIELDS_PTR    field;
    const SE_Image_Fields *img_field = NULL;
          SE_Status_Code   status = SE_STATCODE_SUCCESS;

    if (!start_texel || !stop_texel || !byte_count_out_ptr)
    {
        fprintf(stderr, "SE_GetSizeOfImageData: null param\n");
        status = SE_STATCODE_INACTIONABLE_FAILURE;
    }
    else
    {
        *byte_count_out_ptr = 0;

        if (SE_GetEncoding (image, &encoding) != SE_RETCOD_SUCCESS ||
            SE_CreateStore(encoding, &store) != SE_RETCOD_SUCCESS )
        {
            fprintf(stderr, "SE_GetSizeOfImageData: can't create store\n");
            status = SE_STATCODE_INACTIONABLE_FAILURE;
        }
        else if (SE_GetFields(image, store, &field) != SE_RETCOD_SUCCESS ||
                 field->tag != SE_CLS_DRM_IMAGE )
        {
            fprintf(stderr, "SE_GetSizeOfImageData: can't get fields\n");
            status = SE_STATCODE_INACTIONABLE_FAILURE;
        }
        else
        {
            img_field = &field->u.Image;

            if (SE_ValidImageExtents(img_field, start_texel, stop_texel,
                 mip_level) == SE_FALSE )
            {
                fprintf(stderr,
                        "SE_GetSizeOfImageData: invalid image extents\n");
                status = SE_STATCODE_INACTIONABLE_FAILURE;
            }
            else
            {
                SE_Integer_Unsigned num_texels, num_bytes, bits_per_texel;

                num_texels = (stop_texel->horizontal -
                              start_texel->horizontal + 1) *
                             (stop_texel->vertical -
                              start_texel->vertical   + 1);

                if (img_field->data_is_3D == SE_TRUE)
                    num_texels *= (stop_texel->z - start_texel->z + 1);

                if (SE_BitsPerTexel(img_field, &bits_per_texel) !=
                    SE_STATCODE_SUCCESS)
                {
                    status = SE_STATCODE_INACTIONABLE_FAILURE;
                }
                ...
                {
                    num_bytes = (num_texels / BITS_IN_A_BYTE) * bits_per_texel
                         + ((num_texels % BITS_IN_A_BYTE) * bits_per_texel+ 7)
                                / BITS_IN_A_BYTE;
                    *byte_count_out_ptr = num_bytes;
                }
            }
        }
    }
