...
//
// FILE       : SE_AggregateIterator.cpp
//
// PROGRAMMERS: Robert D'Urso (SAIC), Howard Lu (SAIC),
//              Bill Horan (SAIC), Warren Macchi (Arteon)
//
// DESCRIPTION: Methods for AggregateIterator, a support class.
//              An aggregate iterator is a class that allows the user
//              to search the aggregates of an object.
//
// - API spec. 4.1
...
SE_AggregateIterator::SE_AggregateIterator
(
...
{
    _iter =  root()->get_aggregates_iter( itr_bhvr );
}
...
SE_Status_Code
SE_AggregateIterator::length(SE_Integer_Unsigned *len)
{
...
//------------------------------------------------------------------------------
//
// FUNCTION: SE_AggregateIterator::next_object
//
//   Returns the next object from the Aggregate Iterator.  This Aggregate
//   Iterator was initialized from a root object.  The next object for this
//   Aggregate Iterator is the next approved object off the list of that root's
//   Aggregate objects.  Approved means that the object must satisfy the rules
//   specified in the Search Filter when this Aggregate Iterator was created.
//
//------------------------------------------------------------------------------
SE_Status_Code
SE_AggregateIterator::next_object
(
...
)
{
    ...
    // we continue where we left off, trying to find a matching object
