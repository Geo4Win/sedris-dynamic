...
//
// FILE       : SE_BaseLocation.cpp
//
// PROGRAMMERS: Howard Lu (SAIC), Warren Macchi (Arteon)
//
// DESCRIPTION: Methods for Base <Location> classes.
//              Vendors implementing any subclasses of Locations must
//              implement them as subclasses to this SE_BaseLocation
//              class.
//
// - API spec. 4.1
//
...
//--------------------------------------------------------------------
// METHOD: SE_BaseLocation::~SE_BaseLocation
//
//   Destructor for SE_BaseLocation class; deletes all the cached
//   fields.
//
//--------------------------------------------------------------------
SE_BaseLocation::~SE_BaseLocation()
{
    CacheLocationFields *curr_ptr = NULL, *tmp = NULL;
...
//--------------------------------------------------------------------
//
// FUNCTION: SE_BaseLocation::fields
//
    ...
    *status_ptr      = SE_STATCODE_SUCCESS;
        ...
        */
        ...
        for (tmp = _cache_fieldList; tmp; tmp = tmp->next)
        {
                ...
                (context->get_xform() == tmp->xform))
            {
                if (!tmp->srf_info ||
                    (tmp->srf_info &&
                     SE_SRFContextInfoEqual( raif_mngr()->GetToSRFInfo(), tmp->srf_info)) )
                {
                   ...
                   *not_orig_fields = SE_TRUE;
                    ...
                    if (temp_fields)
                    {
                        ...
                        SE_CloneFields(&tmp->fields,temp_fields);
        ...
        if (SE_CloneFields(OrigFields(), &new_fields) != SE_DRM_STAT_CODE_SUCCESS)
        {
            *status_ptr = SE_STATCODE_OUT_OF_MEMORY;
        }
        else
        ...
        if (OBJSTATUS_VALID(*status_ptr) == SE_TRUE)
        {
            ...
            *status_ptr  = context->apply_transform
            ...
            if (OBJSTATUS_VALID(*status_ptr))
            {
               ...
               // convert location to different SRF if needed
                if (!raif_mngr()->UseDefaultSRFInfo())
                {
                    *status_ptr = context->convert_location(&new_fields);
        ...
        if (*not_orig_fields)
        {
            ...
            CacheLocationFields *newConv = new CacheLocationFields;
            newConv->srf_info = new SE_SRF_Context_Info;

            if (!newConv || !newConv->srf_info ||
                (SE_CloneFields(&new_fields, &(newConv->fields)) != SE_DRM_STAT_CODE_SUCCESS))
            {
               *not_orig_fields = SE_FALSE;
               *status_ptr     = SE_STATCODE_OUT_OF_MEMORY;
            }
    ...
    if (*not_orig_fields)
    {
        ...
        if (temp_fields)
        {
            ...
            SE_CloneFields(&_cache_fieldList->fields,temp_fields);
